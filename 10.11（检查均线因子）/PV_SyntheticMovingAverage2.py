...
def applyParallel(dfGrouped,func,MA):
    ...
    return ma_one_day
#%%
class PV_SyntheticMovingAverage2(StockFactor):
    """
    SyntheticMovingAverage2 (综合移动平均线)
    国泰君安证券“综合期限多样性的趋势选股策略”数量化专题之九十
    
    计算方法: 
    逐日算出各日级均线
    标记出换手率过高（大于当日所有股票中位数、大于自己近12日均值）的股票及日期
    将每日收益率与2日前各日级均线数据线性回归，得到每日的回归系数，回归时去除换手率过高的数据
    对回归系数进行平滑处理，即对过去22日取均值（不包括当日），再用当日的回归系数与均线预测后天的收益，并以此作为因子值
    """
    ...
    def compute(self, start_date, end_date):
        ...
        # 将换手率高于当日平均水平且换手率高于自己前12天平均水平的数据剔除
        turnover = EquityData.get_stocks_yield(start_date = new_start_date,end_date = end_date,fields = ['n_turnoverD'])
        turnover_daymedian = turnover.groupby('t_tradingDate')['n_turnoverD'].median()
        turnover_judge1 = turnover.pivot('c_code', 't_tradingDate', 'n_turnoverD') - turnover_daymedian
        turnover_judge1 = turnover_judge1.unstack().reset_index().rename(columns = {0:'横向'})
        turnover_judge1['横向'] = turnover_judge1['横向'] > 0
        turnover_judge2 = turnover.pivot('t_tradingDate', 'c_code', 'n_turnoverD').rolling(12, 12).mean().dropna(how = 'all')
        turnover_judge2 = (turnover.pivot('t_tradingDate', 'c_code', 'n_turnoverD') - turnover_judge2).dropna(how = 'all')
        turnover_judge2 = turnover_judge2.unstack().reset_index().rename(columns = {0:'纵向'})
        turnover_judge2['纵向'] = turnover_judge2['纵向'] > 0
        turnover_judge = turnover_judge1.merge(turnover_judge2)
        turnover_judge['过高'] = turnover_judge['横向'] & turnover_judge['纵向']
        turnover_judge = turnover_judge[['t_tradingDate', 'c_code', '过高']]
        data = data.merge(turnover_judge)
        data = data[data['过高'] == False]
        data.drop('过高', axis = 1, inplace = True)
...
def get_factor(param=None):
    factor = PV_SyntheticMovingAverage2(
                name='PV_SyntheticMovingAverage',
                description='综合期限均线因子2',
