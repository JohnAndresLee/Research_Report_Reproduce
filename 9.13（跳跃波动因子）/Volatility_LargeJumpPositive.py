...
# %%
class Volatility_LargeJumpPositive(StockFactor):
    """
    大程上行跳跃波动因子
    计算方法：
    ...
    def compute_one_day(self, data_group, date):
        ...
        # Rename
        RLJVP_t.rename(columns = {'c_code':'c_asset'}, inplace = True)
        RLJVP_t['t_date'] = date
...
def get_factor(param=None):
    factor = Volatility_LargeJumpPositive(name='Volatility_LargeJumpPositive'
                                    , category=FactorCategory.Volatility
                                    , description='大程上行跳跃波动因子'
                                    , n = 3)
    return factor
