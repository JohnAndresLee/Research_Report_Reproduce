...
# %%
class Volatility_Jump(StockFactor):
    """
    已实现跳跃波动因子
    计算方法：
    1. 已实现波动率：当天所有高频区间内，股价对数收益率的平方和
    2. 积分波动率：用股价对数收益率的多幂次变差近似
    3. 已实现跳跃波动：已实现波动率减去积分波动率
    ...
    def compute_one_day(self, data_group, date):
        ...
        # Rename
        RJV_t.rename(columns = {'c_code':'c_asset'}, inplace = True)
        RJV_t['t_date'] = date
...
def get_factor(param=None):
    factor = Volatility_Jump(name='Volatility_Jump'
                                    , category=FactorCategory.Volatility
                                    , description='已实现跳跃波动因子'
                                    , n = 3)
    return factor
