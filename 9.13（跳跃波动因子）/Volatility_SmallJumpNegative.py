...
# %%
class Volatility_SmallJumpNegative(StockFactor):
    """
    小程下行跳跃波动因子
    计算方法：
    ...
    def compute_one_day(self, data_group, date):
        ...
        # Rename
        RSJVN_t.rename(columns = {'c_code':'c_asset'}, inplace = True)
        RSJVN_t['t_date'] = date
...
def get_factor(param=None):
    factor = Volatility_SmallJumpNegative(name='Volatility_SmallJumpNegative'
                                    , category=FactorCategory.Volatility
                                    , description='小程下行跳跃波动因子'
                                    , n = 3)
    return factor
