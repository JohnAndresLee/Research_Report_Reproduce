...
# %%
class Volatility_AsymmetricalSmallJump(StockFactor):
    """
    小程上下行跳跃波动不对称性因子
    计算方法：
    1. 大程跳跃的阈值gamma：gamma = alpha * pow(N, -0.49) * pow(IV, 0.5)
    2. 大程上（下）行跳跃序列：对数收益率 >(< -) 阈值
    3. 大程上（下）行跳跃波动：大程上（下）行跳跃序列的对数收益平方和
    4. 小程上（下）行跳跃波动：上（下）行跳跃波动 - 大程上（下）行跳跃波动
    5. 小程上下行跳跃波动不对称：小程上行跳跃波动 - 小程下行跳跃波动
    ...
    def compute_one_day(self, data_group, date):
        ...
        SRLJV_t = RLJVP_t.copy(deep = True)
        SRLJV_t['n_value'] = SRLJV_t['n_value'] - RLJVN_t['n_value']

        # RSJVP_t, RSJVN_t, SRSJV_t
        RSJVP_t = RJVP_t.copy(deep = True)
        RSJVP_t['n_value'] = RSJVP_t['n_value'] - RLJVP_t['n_value']
        RSJVN_t = RJVN_t.copy(deep = True)
        RSJVN_t['n_value'] = RSJVN_t['n_value'] - RLJVN_t['n_value']
        SRSJV_t = RSJVP_t.copy(deep = True)
        SRSJV_t['n_value'] = SRSJV_t['n_value'] - RSJVN_t['n_value']


        # Rename
        SRSJV_t.rename(columns = {'c_code':'c_asset'}, inplace = True)
        SRSJV_t['t_date'] = date
...
def get_factor(param=None):
    factor = Volatility_AsymmetricalSmallJump(name='Volatility_AsymmetricalSmallJump'
                                    , category=FactorCategory.Volatility
                                    , description='小程上下行跳跃波动不对称性因子'
                                    , n = 3)
    return factor
