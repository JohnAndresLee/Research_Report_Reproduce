...
# %%
class Volatility_AsymmetricalLargeJump(StockFactor):
    """
    大程上下行跳跃波动不对称性因子
    计算方法：
    1. 大程跳跃的阈值gamma：gamma = alpha * pow(N, -0.49) * pow(IV, 0.5)
    2. 大程上（下）行跳跃序列：对数收益率 >(< -) 阈值 gamma
    3. 大程上（下）行跳跃波动：大程上（下）行跳跃序列的对数收益平方和
    4. 大程上下行跳跃波动不对称：大程上行跳跃波动 - 大程下行跳跃波动
    ...
    def compute_one_day(self, data_group, date):
        ...
        # Rename
        SRLJV_t.rename(columns = {'c_code':'c_asset'}, inplace = True)
        SRLJV_t['t_date'] = date
...
def get_factor(param=None):
    factor = Volatility_AsymmetricalLargeJump(name='Volatility_AsymmetricalLargeJump'
                                    , category=FactorCategory.Volatility
                                    , description='大程上下行跳跃波动不对称性'
                                    , n = 3)
    return factor
