...
# %%
class Volatility_JumpPositive(StockFactor):
    """
    上行跳跃波动因子
    计算方法：上行波动率 - 积分波动率的一半
    ...
    def compute_one_day(self, data_group, date):
        ...
        # Rename
        RJVP_t.rename(columns = {'c_code':'c_asset'}, inplace = True)
        RJVP_t['t_date'] = date
...
def get_factor(param=None):
    factor = Volatility_JumpPositive(name='Volatility_JumpPositive'
                                    , category=FactorCategory.Volatility
                                    , description='上行跳跃波动因子'
                                    , n = 3)
    return factor
