...
# %%
class Volatility_LargeJump(StockFactor):
    """
    大程跳跃波动因子
    计算方法：
    1. 大程跳跃的阈值gamma：gamma = alpha * pow(N, -0.49) * pow(IV, 0.5)
    2. 大程跳跃序列：对数收益率绝对值 > 阈值 gamma
    3. 大程跳跃：大程跳跃序列的对数收益平方和
    ...
    def compute_one_day(self, data_group, date):
        ...
        # Rename
        RLJV_t.rename(columns = {'c_code':'c_asset'}, inplace = True)
        RLJV_t['t_date'] = date
...
def get_factor(param=None):
    factor = Volatility_LargeJump(name='Volatility_LargeJump'
