...
# %%
class Volatility_JumpNegative(StockFactor):
    """
    下行跳跃波动因子
    计算方法：下行波动率 - 积分波动率的一半
    ...
    def compute_one_day(self, data_group, date):
        ...
        # Rename
        RJVN_t.rename(columns = {'c_code':'c_asset'}, inplace = True)
        RJVN_t['t_date'] = date
...
def get_factor(param=None):
    factor = Volatility_JumpNegative(name='Volatility_JumpNegative'
                                    , category=FactorCategory.Volatility
                                    , description='下行跳跃波动因子'
                                    , n = 3)
    return factor
