...
# %%
class Volatility_SmallJumpPositive(StockFactor):
    """
    小程上行跳跃波动因子
    计算方法：
    1. 大程跳跃的阈值gamma：gamma = alpha * pow(N, -0.49) * pow(IV, 0.5)
    2. 大程上行跳跃序列：对数收益率 > 阈值 gamma
    3. 大程上行跳跃波动：大程上行跳跃序列的对数收益平方和
    4. 小程上行跳跃波动：上行跳跃波动 - 大程上行跳跃波动
    ...
    def compute_one_day(self, data_group, date):
        ...
        # Rename
        RSJVP_t.rename(columns = {'c_code':'c_asset'}, inplace = True)
        RSJVP_t['t_date'] = date
...
def get_factor(param=None):
    factor = Volatility_SmallJumpPositive(name='Volatility_SmallJumpPositive'
                                    , category=FactorCategory.Volatility
                                    , description='小程上行跳跃波动'
                                    , n = 3)
    return factor
