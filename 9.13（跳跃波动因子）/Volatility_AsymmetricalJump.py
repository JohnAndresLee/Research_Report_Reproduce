...
# %%
class Volatility_AsymmetricalJump(StockFactor):
    """
    上下行跳跃波动不对称性因子
    计算方法：
    1. 上行跳跃波动：上行波动率 - 积分波动率的一半
    2. 下行跳跃波动：下行波动率 - 积分波动率的一半
    3. 上下行跳跃波动不对称性：上行跳跃波动 - 下行跳跃波动
    ...
    def compute_one_day(self, data_group, date):
        ...
        # Rename
        SRJV_t.rename(columns = {'c_code':'c_asset'}, inplace = True)
        SRJV_t['t_date'] = date
...
def get_factor(param=None):
    factor = Volatility_AsymmetricalJump(name='Volatility_AsymmetricalJump'
                                    , category=FactorCategory.Volatility
                                    , description='上下行跳跃波动不对称性'
                                    , n = 3)
    return factor
